...
# ======================================================
# Obstáculos rectangulares y evasión (1..2 obstáculos)
# ======================================================
def inflate_rect(rect, margin):
    """rect=(x0,y0,w,h) -> (x0m,y0m,x1m,y1m) inflado."""
    x0, y0, w, h = rect
    x1 = x0 + w
    y1 = y0 + h
    x0m = x0 - margin
...
def segment_intersects_rect(p0, p1, r):
    """Intersección de segmento (axis-aligned esperado) con rect r=(x0,y0,x1,y1)."""
    x0, y0 = p0
    x1, y1 = p1

    rx0, ry0, rx1, ry1 = r

    if point_in_rect(x0, y0, r) or point_in_rect(x1, y1, r):
        return True
    ...
    if abs(x1 - x0) < 1e-12:
        ...
        if rx0 <= x <= rx1:
            a = min(y0, y1)
            b = max(y0, y1)
            return not (b < ry0 or a > ry1)
        return False

    # Caso no axis-aligned: bbox conservador
    a0 = min(x0, x1)
    a1 = max(x0, x1)
    b0 = min(y0, y1)
    b1 = max(y0, y1)
    return not (a1 < rx0 or a0 > rx1 or b1 < ry0 or b0 > ry1)


def path_intersects_any_obstacle(path_pts, inflated_rects):
    if len(path_pts) < 2:
        return False
    for i in range(len(path_pts) - 1):
        ...
        for r in inflated_rects:
            if segment_intersects_rect(p0, p1, r):
                return True
    return False


def try_detours_for_first_hit(ax, ay, bx, by, step_m, x_first, inflated_rects, bounds, max_candidates=12):
    """Propone rutas con 1 desvío alrededor del primer obstáculo encontrado."""
    Xmin, Xmax, Ymin, Ymax = bounds

    wps = build_manhattan_waypoints(ax, ay, bx, by, step_m, x_first=x_first)
    base = [(ax, ay)] + wps
    if not path_intersects_any_obstacle(base, inflated_rects):
        return base
    ...
    rx0, ry0, rx1, ry1 = hit_rect

    eps = 0.001
    x_left = clamp_float(rx0 - eps, Xmin, Xmax)
    x_right = clamp_float(rx1 + eps, Xmin, Xmax)
    y_down = clamp_float(ry0 - eps, Ymin, Ymax)
    y_up = clamp_float(ry1 + eps, Ymin, Ymax)

    candidates = []

    for x_det in [x_left, x_right]:
        candidates.append([(ax, ay), (x_det, ay), (x_det, by), (bx, by)])
        candidates.append([(ax, ay), (x_det, ay), (x_det, y_down), (bx, y_down), (bx, by)])
        candidates.append([(ax, ay), (x_det, ay), (x_det, y_up), (bx, y_up), (bx, by)])

    for y_det in [y_down, y_up]:
        candidates.append([(ax, ay), (ax, y_det), (bx, y_det), (bx, by)])
        candidates.append([(ax, ay), (x_left, y_det), (x_left, by), (bx, by)])
        candidates.append([(ax, ay), (x_right, y_det), (x_right, by), (bx, by)])

    uniq = []
    seen = set()
    for cand in candidates:
        key = tuple((round(p[0], 4), round(p[1], 4)) for p in cand)
        if key not in seen:
            seen.add(key)
            uniq.append(cand)
    candidates = uniq[:max_candidates]

    def expand_polyline(points):
        out = [points[0]]
        for k in range(len(points) - 1):
            x0, y0 = out[-1]
            x1, y1 = points[k + 1]
            if abs(x1 - x0) > 1e-12:
                tmp = build_manhattan_waypoints(x0, y0, x1, y0, step_m, x_first=True)
                out.extend(tmp)
                x0, y0 = out[-1]
            if abs(y1 - y0) > 1e-12:
                tmp = build_manhattan_waypoints(x0, y0, x0, y1, step_m, x_first=True)
                out.extend(tmp)
        return out

    for cand in candidates:
        cand2 = [(clamp_float(px, Xmin, Xmax), clamp_float(py, Ymin, Ymax)) for px, py in cand]
        path_pts = expand_polyline(cand2)
        if not path_intersects_any_obstacle(path_pts, inflated_rects):
            return path_pts

    return None


def build_manhattan_waypoints_avoiding(ax, ay, bx, by, step_m, x_first, obstacles_rects, clearance_m):
    """Waypoints Manhattan evitando hasta 2 obstáculos rectangulares (heurística de desvío)."""
    ...
    inflated = [inflate_rect(r, clearance_m) for r in rects]

    path_pts = [(ax, ay)] + build_manhattan_waypoints(ax, ay, bx, by, step_m, x_first=x_first)
    if not path_intersects_any_obstacle(path_pts, inflated):
        return path_pts[1:]

    current = (ax, ay)
    target = (bx, by)
    remaining_inflated = inflated[:]
    for _ in range(2):
        trial = try_detours_for_first_hit(
            current[0], current[1], target[0], target[1],
            step_m=step_m, x_first=x_first,
            inflated_rects=remaining_inflated,
            bounds=bounds
        )
        if trial is None:
            break
        if not path_intersects_any_obstacle(trial, remaining_inflated):
            return trial[1:]
        path_pts = trial

    alt = [(ax, ay)] + build_manhattan_waypoints(ax, ay, bx, by, step_m, x_first=not x_first)
    if not path_intersects_any_obstacle(alt, inflated):
        return alt[1:]

    return build_manhattan_waypoints(ax, ay, bx, by, step_m, x_first=x_first)


# ======================================================
# Hilo de movimiento (multicadena) - ENVÍO SINCRONIZADO
# + Parada periódica para "medición"
# + Reinicio de seno por cadena al finalizar, al HOME o tras medir
# + Giro: aplica cambio de fase SOLO en un sentido (configurable)
# ======================================================
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                ...
                # medición
                meas_every = float(self.app.measure_every_s_var.get())
                meas_hold = float(self.app.measure_hold_s_var.get())
                meas_every = max(0.0, meas_every)
                meas_hold = max(0.0, meas_hold)

                # giro: fase adicional SOLO en un sentido
                shift_enable = bool(self.app.turn_phase_shift_enable.get())
                shift_dir = self.app.turn_phase_shift_dir.get().strip()  # "CW" o "CCW"
                ...
                for c in self.app.chains_runtime:
                    ...
                    # -------------------------
                    # Parada periódica (medición)
                    # -------------------------
                    if meas_every > 0 and meas_hold > 0:
                        # Programación por cadena
                        if c.get("next_measure_ts") is None:
                            c["next_measure_ts"] = now + meas_every

                        # Si está midiendo: mantener HOME y esperar
                        if c.get("measuring", False):
                            if now >= c.get("measure_end_ts", now):
                                # termina medición -> reinicia variables (seno)
                                c["measuring"] = False
                                c["t0_chain"] = time.perf_counter()
                                c["next_measure_ts"] = now + meas_every
                            else:
                                # mantener servo en HOME para simular detenido
                                self.app.after(0, lambda ids=c["ids"]: self.app.home_ids(ids))
                                continue

                        # Entrar a medición
                        if now >= c["next_measure_ts"]:
                            c["measuring"] = True
                            c["measure_end_ts"] = now + meas_hold
                            # detener: HOME inmediato
                            self.app.after(0, lambda ids=c["ids"]: self.app.home_ids(ids))
                            continue
                    ...
                    if wp is None:
                        c["done"] = True
                        c["t0_chain"] = time.perf_counter()  # reinicio seno
                        self.app.after(0, lambda ids=c["ids"]: self.app.home_ids(ids))
                        continue
                    ...
                    cw = err < 0  # cw=True suele interpretarse como giro a la derecha

                    # amplitudes y fases base
                    if turning:
                    ...
                    # -------------------------
                    # Cambio de fase SOLO en un sentido de giro
                    # (en el otro sentido, no cambia)
                    # Se aplica a la fase H (horizontal) durante giro.
                    # -------------------------
                    extra_phi_h = 0.0
                    ...
                    # tiempo local de cadena (reiniciable)
                    t_chain = time.perf_counter() - c.get("t0_chain", time.perf_counter())
...
class PlotWindow(tk.Toplevel):
    def __init__(self, app):
        ...
        self.h_scatter = None
        self.h_max_pt, = self.ax.plot([], [], marker="*", markersize=14)
    ...
    def _draw_obstacles(self):
        ...
        for ob in self.app.get_obstacles_rects():
            if ob is None:
                continue
            x0, y0, w, h = ob
            if w <= 0 or h <= 0:
                continue
            r = Rectangle((x0, y0), w, h, fill=False, linewidth=2.0)
            self.ax.add_patch(r)
            self.ob_patches.append(r)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Fase adicional SOLO en un sentido de giro
        self.turn_phase_shift_enable = tk.BooleanVar(value=True)
        self.turn_phase_shift_dir = tk.StringVar(value="CW")  # "CW" (derecha) o "CCW" (izquierda)
        self.turn_phase_shift_deg = tk.StringVar(value="45")  # grados

        # Parada periódica para medición
        self.measure_every_s_var = tk.StringVar(value="6")   # cada N segundos
        self.measure_hold_s_var = tk.StringVar(value="1.0")  # detener M segundos
        ...
        # --- Obstáculos (hasta 2) ---
        self.obs_clearance_var = tk.StringVar(value="0.06")

        self.obs1_on = tk.BooleanVar(value=False)
        self.obs1_x = tk.StringVar(value="0.60")
        self.obs1_y = tk.StringVar(value="0.35")
        self.obs1_w = tk.StringVar(value="0.35")
        self.obs1_h = tk.StringVar(value="0.25")

        self.obs2_on = tk.BooleanVar(value=False)
        self.obs2_x = tk.StringVar(value="1.10")
        self.obs2_y = tk.StringVar(value="0.70")
        self.obs2_w = tk.StringVar(value="0.30")
        self.obs2_h = tk.StringVar(value="0.20")
    ...
    def _build_ui(self):
        ...
        self._ui_measure(col2)
        self._ui_model(col2)

        self._ui_obstacles(col3)
    ...
    def _ui_measure(self, parent):
        f = ttk.LabelFrame(parent, text="Paradas periódicas para medición", padding=10)
        f.pack(fill="x", pady=(0, 10))

        ttk.Label(f, text="Cada (s)").grid(row=0, column=0, sticky="w")
        ttk.Entry(f, textvariable=self.measure_every_s_var, width=10).grid(row=0, column=1, padx=6, sticky="w")
        ttk.Label(f, text="Detener (s)").grid(row=0, column=2, sticky="w")
        ttk.Entry(f, textvariable=self.measure_hold_s_var, width=10).grid(row=0, column=3, padx=6, sticky="w")
        ttk.Label(f, text="Acción: HOME durante medición + reinicio seno al reanudar").grid(row=1, column=0, columnspan=4, sticky="w", pady=(6, 0))
    ...
    def _ui_obstacles(self, parent):
        f = ttk.LabelFrame(parent, text="Obstáculos (rectángulos) - evasión Manhattan", padding=10)
        f.pack(fill="x", pady=(0, 10))

        ttk.Label(f, text="Margen seguridad (m)").grid(row=0, column=0, sticky="w")
        ttk.Entry(f, textvariable=self.obs_clearance_var, width=10).grid(row=0, column=1, padx=6, sticky="w")
        ttk.Label(f, text="(se infla el rectángulo)").grid(row=0, column=2, columnspan=2, sticky="w")

        ttk.Checkbutton(f, text="Obstáculo 1", variable=self.obs1_on).grid(row=1, column=0, sticky="w", pady=(8, 0))
        ttk.Label(f, text="x").grid(row=1, column=1, sticky="e", pady=(8, 0))
        ttk.Entry(f, textvariable=self.obs1_x, width=8).grid(row=1, column=2, sticky="w", pady=(8, 0))
        ttk.Label(f, text="y").grid(row=1, column=3, sticky="e", pady=(8, 0))
        ttk.Entry(f, textvariable=self.obs1_y, width=8).grid(row=1, column=4, sticky="w", pady=(8, 0))
        ttk.Label(f, text="w").grid(row=2, column=1, sticky="e")
        ttk.Entry(f, textvariable=self.obs1_w, width=8).grid(row=2, column=2, sticky="w")
        ttk.Label(f, text="h").grid(row=2, column=3, sticky="e")
        ttk.Entry(f, textvariable=self.obs1_h, width=8).grid(row=2, column=4, sticky="w")

        ttk.Checkbutton(f, text="Obstáculo 2", variable=self.obs2_on).grid(row=3, column=0, sticky="w", pady=(8, 0))
        ttk.Label(f, text="x").grid(row=3, column=1, sticky="e", pady=(8, 0))
        ttk.Entry(f, textvariable=self.obs2_x, width=8).grid(row=3, column=2, sticky="w", pady=(8, 0))
        ttk.Label(f, text="y").grid(row=3, column=3, sticky="e", pady=(8, 0))
        ttk.Entry(f, textvariable=self.obs2_y, width=8).grid(row=3, column=4, sticky="w", pady=(8, 0))
        ttk.Label(f, text="w").grid(row=4, column=1, sticky="e")
        ttk.Entry(f, textvariable=self.obs2_w, width=8).grid(row=4, column=2, sticky="w")
        ttk.Label(f, text="h").grid(row=4, column=3, sticky="e")
        ttk.Entry(f, textvariable=self.obs2_h, width=8).grid(row=4, column=4, sticky="w")
    ...
    # ================= Obstáculos helpers =================
    def get_obstacles_rects(self):
        rects = []

        def read_rect(on, sx, sy, sw, sh):
            if not on.get():
                return None
            try:
                x = float(sx.get()); y = float(sy.get())
                w = float(sw.get()); h = float(sh.get())
            except Exception:
                return None
            x = clamp_float(x, X_MIN, X_MAX)
            y = clamp_float(y, Y_MIN, Y_MAX)
            w = max(0.0, min(w, X_MAX - x))
            h = max(0.0, min(h, Y_MAX - y))
            return (x, y, w, h)

        rects.append(read_rect(self.obs1_on, self.obs1_x, self.obs1_y, self.obs1_w, self.obs1_h))
        rects.append(read_rect(self.obs2_on, self.obs2_x, self.obs2_y, self.obs2_w, self.obs2_h))
        return rects

    def get_clearance(self):
        try:
            c = float(self.obs_clearance_var.get())
        except Exception:
            c = 0.06
        return max(0.0, min(c, 0.50))

    # ================= Humedad: opción 1 (bloque rojo centrado en B común) =================
    def generate_humidity_map(self):
        ...
        self.set_status(f"Mapa generado. Máximo en ({mx:.2f},{my:.2f}) h={mh:.3f}.")
    ...
    def home_all(self):
        ...
        # reinicio de variables por cadena (seno + estado de medición)
        now = time.perf_counter()
        for c in self.chains_runtime:
            c["t0_chain"] = now
            c["measuring"] = False
            c["next_measure_ts"] = None
            c["measure_end_ts"] = None

        self.home_ids(sorted(ids))
        self.set_status(f"HOME enviado a {sorted(ids)} (reinicio seno).")
    ...
    def _build_runtime_from_ui(self):
        ...
        obstacles = self.get_obstacles_rects()
        ...
        for k in range(n):
            ...
            wps = build_manhattan_waypoints_avoiding(
                ax, ay, bxk, byk,
                step_m=step_m,
                x_first=x_first,
                obstacles_rects=obstacles,
            ...
            chains.append({
                ...
                # reinicio seno + medición
                "t0_chain": time.perf_counter(),
                "measuring": False,
                "next_measure_ts": None,
                "measure_end_ts": None,
            })
    ...
    def start_all(self):
        ...
        self.thread.start()
        self.set_status("Movimiento multicadena iniciado (medición periódica + fase direccional en giro).")
