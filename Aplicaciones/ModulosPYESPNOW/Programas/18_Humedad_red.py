...
class App(tk.Tk):
    ...
    def get_current_humidity_value(self, x, y):
        """
        Lee la humedad del mapa en la posición (x, y) del móvil.
        Retorna un valor normalizado entre 0 y 1.
        """

        # Garantizar que la posición esté dentro del plano
        x, y = self.clip_xy(x, y)

        if not self.hmap.is_ready():
            return 0.0

        # Interpolación de la humedad en la posición del móvil
        humedad_movil = self.hmap.value_bilinear(x, y)

        return float(max(0.0, min(1.0, humedad_movil)))
#    def get_current_humidity_value(self, x, y):
        """
