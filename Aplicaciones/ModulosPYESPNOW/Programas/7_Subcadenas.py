...
class MotionThread(threading.Thread):
    """
    En cada ciclo:
      1) Se calcula el estado (turning/cw/Av/Ah/phi) para cada cadena en el mismo instante.
      2) Se generan TODAS las líneas serial para todos los IDs (t fijo).
      3) Se envía el lote completo, de modo que "todos se muevan al tiempo" en práctica.
    """
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                ...
                # tiempo único para todos en este ciclo
                t = time.perf_counter() - self.t0

                # dt único para actualización de pose
                ...
                batch_lines = []  # TODAS las líneas a enviar en este ciclo

                # Primero: decidir comandos de cada cadena en el mismo instante
                for c in self.app.chains_runtime:
                    ...
                    # Generar líneas para esa cadena (pero NO enviar aún)
                    lines = self._build_lines_chain(
                        chain=c,
                        t=t,
                        freq=freq,
                        Av=Av,
                    ...
                    # Actualizar pose de esa cadena con dt común
                    self._update_pose_model_chain(c, Av, Ah, turning, cw, kv, kw, dt)

                    # Registrar trayectoria (real) para graficar
                ...
                # Segundo: enviar TODO el lote en un solo ciclo (sincronizado)
                if batch_lines:
    ...
    def _build_lines_chain(self, chain, t, freq, Av, Ah, phi_v_deg, turning, cw):
        # Sentido global (si se invierte en campo, cambie el signo)
        sign_v = -1.0
        if turning:
...
# ======================================================
# Ventana de gráfica (Toplevel) multicadena
#  - solo punta orientada (triángulo)
#  - etiqueta con # módulos al lado
#  - color distinto por cadena
# ======================================================
class PlotWindow(tk.Toplevel):
    def __init__(self, app):
        super().__init__(app)
        self.app = app
        self.title("Recorridos X–Y (REAL, m) | Multicadena")
        ...
        # Colores por cadena (hasta 3)
        self.chain_colors = ["tab:blue", "tab:orange", "tab:green"]
    ...
    def _refresh(self):
        ...
        try:
            ...
            for i, c in enumerate(self.app.chains_runtime):
                ...
                axr, ayr = c["A"]
                if c["waypoints"]:
                    xs = [axr] + [p[0] for p in c["waypoints"]]
                    ys = [ayr] + [p[1] for p in c["waypoints"]]
                    self.chain_plan_lines[i].set_data(xs, ys)
                else:
                ...
                dx = 0.03
                dy = 0.02
                self.chain_labels[i].set_position((x_r + dx, y_r + dy))
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Multicadena (1..3) | A→B por rejilla | Envío sincronizado")
        ...
        # Punto final común
        self.bx_var = tk.StringVar(value="0.50")
        self.by_var = tk.StringVar(value="0.30")

        # Horizontales globales (por módulo, 1..6)
        self.horiz_vars = {i: tk.BooleanVar(value=False) for i in range(1, 7)}

        # --- Config multicadena (hasta 3) ---
    ...
    def _ui_model(self, parent):
        f = ttk.LabelFrame(parent, text="Modelo + escala", padding=10)
        f.pack(fill="x", pady=(0, 10))
        ...
        ttk.Label(f, text="Ej: 0.01 => 1 modelo = 1 cm").grid(row=2, column=0, columnspan=4, sticky="w", pady=(6, 0))
    ...
    def _ui_exec(self, parent):
        ...
        ttk.Button(f, text="Iniciar (todas → B)", command=self.start_all).pack(side="left", padx=6)
    ...
    def _ui_status(self, parent):
        ...
        f.pack(fill="x")
        ttk.Label(f, textvariable=self.status_var, wraplength=450, justify="left").pack(anchor="w")
    ...
    def send_lines_batch(self, lines):
        """Envía todas las líneas en un solo bloqueo de escritura."""
        if not self.ser or not self.ser.is_open:
            raise RuntimeError("Maestro no conectado")
