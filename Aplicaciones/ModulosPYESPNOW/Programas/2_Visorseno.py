...
class MasterSerialWorker(threading.Thread):
    ...
    Permite enviar comandos al Maestro (que reenviará por ESP-NOW/BLE según su firmware).
    """
...
class SineSender(threading.Thread):
    """
    Genera setpoints senoidales y envía al maestro en el formato:
      - Dynamixel (1..3): N <id> POS <pos_int>
      - LX16A (4..6):     N <id> LX  <ang_int>
    """
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                freq = float(self.app.sine_freq_var.get())
                rate = float(self.app.sine_rate_var.get())
                if freq <= 0 or rate <= 0:
                    time.sleep(0.1)
                    continue

                amp_dx = float(self.app.sine_amp_dx_var.get())
                amp_lx = float(self.app.sine_amp_lx_var.get())

                t = time.perf_counter() - self.t0

                for rid in range(1, 7):
                    if not self.app.sine_enable_vars[rid].get():
                        continue

                    try:
                        phase_deg = float(self.app.sine_phase_vars[rid].get())
                    except Exception:
                        phase_deg = 0.0

                    if rid in (1, 2, 3):
                        center = 512
                        raw = center + amp_dx * math.sin(2 * math.pi * freq * t + math.radians(phase_deg))
                        pos = int(round(raw))
                        pos = max(0, min(1023, pos))
                        self.app._send_to_master(f"N {rid} POS {pos}")
                        self.app.sine_status_var.set(f"Seno: N {rid} POS {pos}")
                    else:
                        center = 95
                        raw = center + amp_lx * math.sin(2 * math.pi * freq * t + math.radians(phase_deg))
                        ang = int(round(raw))
                        ang = max(0, min(240, ang))
                        self.app._send_to_master(f"N {rid} LX {ang}")
                        self.app.sine_status_var.set(f"Seno: N {rid} LX {ang}")

                time.sleep(1.0 / rate)

            except Exception as e:
                self.app.sine_status_var.set(f"Seno detenido por error: {e}")
                self.stop_event.set()
                break
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Senoidal
        self.sine_stop_event = threading.Event()
        self.sine_worker = None
    ...
    def _build_ui(self):
        ...
        # Conexión Maestro
        # =======================
        frm_top = ttk.LabelFrame(self, text="Conexión Maestro (ESP32 Central)", padding=4)
        ...
        # Tabla sensores
        # =======================
        frm_mid = ttk.LabelFrame(self, text="Sensores (fila roja si IN < 200)", padding=3)
        frm_mid.pack(fill="both", expand=True, padx=10, pady=(0, 5))

        cols = ("node", "in1", "in2", "in3", "last_seen")
        self.tree = ttk.Treeview(frm_mid, columns=cols, show="headings", height=5)
        ...
        # Control manual
        # =======================
        frm_ctrl = ttk.LabelFrame(self, text="Control manual por robot (motores diferentes)", padding=10)
        ...
        self.lx_ang_var = tk.StringVar(value="95")
        ...
        ttk.Label(self.frm_b, text="(envía: N <id> LX <angulo>)").grid(row=0, column=3, sticky="w", padx=(16, 0))
        ...
        # Control senoidal
        # =======================
        frm_sine = ttk.LabelFrame(self, text="Movimiento senoidal (IDs 1–6) vía Maestro", padding=10)
        frm_sine.pack(fill="x", padx=10, pady=(0, 10))

        ttk.Label(frm_sine, text="Frecuencia (Hz):").grid(row=0, column=0, sticky="w")
        self.sine_freq_var = tk.StringVar(value="0.30")
        ttk.Entry(frm_sine, textvariable=self.sine_freq_var, width=10).grid(row=0, column=1, padx=6, sticky="w")

        ttk.Label(frm_sine, text="Tasa envío (Hz):").grid(row=0, column=2, sticky="w", padx=(16, 0))
        self.sine_rate_var = tk.StringVar(value="30")
        ttk.Entry(frm_sine, textvariable=self.sine_rate_var, width=10).grid(row=0, column=3, padx=6, sticky="w")

        ttk.Label(frm_sine, text="Amp Dynamixel (ticks):").grid(row=0, column=4, sticky="w", padx=(16, 0))
        self.sine_amp_dx_var = tk.StringVar(value="120")
        ttk.Entry(frm_sine, textvariable=self.sine_amp_dx_var, width=10).grid(row=0, column=5, padx=6, sticky="w")

        ttk.Label(frm_sine, text="Amp LX16A (unid):").grid(row=0, column=6, sticky="w", padx=(16, 0))
        self.sine_amp_lx_var = tk.StringVar(value="30")
        ttk.Entry(frm_sine, textvariable=self.sine_amp_lx_var, width=10).grid(row=0, column=7, padx=6, sticky="w")

        self.btn_sine_start = ttk.Button(frm_sine, text="Iniciar seno", command=self._start_sine)
        self.btn_sine_start.grid(row=1, column=0, padx=6, pady=(8, 0), sticky="w")

        self.btn_sine_stop = ttk.Button(frm_sine, text="Detener seno", command=self._stop_sine, state="disabled")
        self.btn_sine_stop.grid(row=1, column=1, padx=6, pady=(8, 0), sticky="w")

        self.btn_sine_plot = ttk.Button(frm_sine, text="Ver gráficas", command=self._show_sine_plot)
        self.btn_sine_plot.grid(row=1, column=7, padx=6, pady=(8, 0), sticky="e")

        self.sine_status_var = tk.StringVar(value="Seno: detenido")
        ttk.Label(frm_sine, textvariable=self.sine_status_var).grid(row=1, column=2, columnspan=5, sticky="w",
                                                                    padx=(12, 0), pady=(8, 0))

        # Fases y enable por robot
        phases_box = ttk.Frame(frm_sine)
        phases_box.grid(row=2, column=0, columnspan=8, sticky="ew", pady=(10, 0))

        ttk.Label(phases_box, text="Habilitar y fase (deg) por robot:").grid(row=0, column=0, columnspan=12, sticky="w")

        self.sine_enable_vars = {}
        self.sine_phase_vars = {}

        default_phases = {1: 0, 2: 60, 3: 120, 4: 180, 5: 240, 6: 300}
        col = 0
        for rid in range(1, 7):
            en = tk.BooleanVar(value=True)
            ph = tk.StringVar(value=str(default_phases[rid]))
            self.sine_enable_vars[rid] = en
            self.sine_phase_vars[rid] = ph

            ttk.Checkbutton(phases_box, text=f"R{rid}", variable=en).grid(row=1, column=col, padx=(0, 6), sticky="w")

            sp = ttk.Spinbox(phases_box, from_=-360, to=360, increment=1, textvariable=ph, width=6)
            sp.grid(row=1, column=col + 1, padx=(0, 14), sticky="w")
    ...
    def _start_sine(self):
        if not self.worker:
            messagebox.showwarning("Senoidal", "Conecte primero el Maestro.")
            return
        if self.sine_worker and self.sine_worker.is_alive():
            return

        try:
            freq = float(self.sine_freq_var.get())
            rate = float(self.sine_rate_var.get())
            amp_dx = float(self.sine_amp_dx_var.get())
            amp_lx = float(self.sine_amp_lx_var.get())
            if freq <= 0 or rate <= 0:
                raise ValueError("Frecuencia y tasa deben ser > 0.")
            if amp_dx < 0 or amp_lx < 0:
                raise ValueError("Amplitudes deben ser >= 0.")
        except Exception as e:
            messagebox.showwarning("Senoidal", str(e))
            return

        self.sine_stop_event.clear()
        self.sine_worker = SineSender(self, self.sine_stop_event)
        self.sine_worker.start()

        self.btn_sine_start.config(state="disabled")
        self.btn_sine_stop.config(state="normal")
        self.sine_status_var.set("Seno: activo")

    def _stop_sine(self):
        if self.sine_worker and self.sine_worker.is_alive():
            self.sine_stop_event.set()
            self.sine_status_var.set("Seno: detenido")
        self.btn_sine_start.config(state="normal")
        self.btn_sine_stop.config(state="disabled")

    # =========================
    # Gráficas (nuevo)
    # =========================
    def _show_sine_plot(self):
        """
        Vista previa teórica de los senos con los parámetros actuales (0..5 s).
        """
        try:
            freq = float(self.sine_freq_var.get())
            if freq <= 0:
                raise ValueError("Frecuencia debe ser > 0.")
            amp_dx = float(self.sine_amp_dx_var.get())
            amp_lx = float(self.sine_amp_lx_var.get())
            if amp_dx < 0 or amp_lx < 0:
                raise ValueError("Amplitudes deben ser >= 0.")
        except Exception as e:
            messagebox.showwarning("Gráficas", str(e))
            return

        T = 5.0
        n = 600
        dt = T / (n - 1)
        tt = [i * dt for i in range(n)]

        win = tk.Toplevel(self)
        win.title("Gráficas senoidales (vista previa)")
        ...
        any_curve = False
        for rid in range(1, 7):
            if not self.sine_enable_vars[rid].get():
                continue

            try:
                phase_deg = float(self.sine_phase_vars[rid].get())
            except Exception:
                phase_deg = 0.0
            ...
            if rid in (1, 2, 3):
                ...
                yy = []
                for t in tt:
                    y = center + amp_dx * math.sin(2 * math.pi * freq * t + math.radians(phase_deg))
                    y = max(0, min(1023, y))
                    yy.append(y)
                ax.plot(tt, yy, label=f"R{rid} (DX)")
                any_curve = True
            else:
                center = 95
                yy = []
                for t in tt:
                    y = center + amp_lx * math.sin(2 * math.pi * freq * t + math.radians(phase_deg))
                    y = max(0, min(240, y))
                    yy.append(y)
                ax.plot(tt, yy, label=f"R{rid} (LX)")
                any_curve = True

        if any_curve:
            ax.legend(loc="upper right", fontsize=8)
        else:
            ax.set_title("No hay robots habilitados para graficar.")
