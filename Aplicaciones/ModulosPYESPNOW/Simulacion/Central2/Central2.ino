...
static const uint32_t SERIAL_BAUD = 460800;  // evita congestión HUB -> Python
static const int8_t WIFI_TX_POWER = 78;      // 19.5 dBm; ajustar según regulación local
static const bool DEBUG_TX_OK = false;       // no mezclar diagnósticos con filas DATA

// Los callbacks de recepción y envío pueden ejecutarse en contextos distintos.
// Este mux evita que dos líneas UART se intercalen.
static SemaphoreHandle_t serialMutex = nullptr;

static void serialLineAtomic(const char *line) {
  if (serialMutex && xSemaphoreTake(serialMutex, pdMS_TO_TICKS(25)) == pdTRUE) {
    Serial.println(line);
...
static void printMeasurementRow(uint8_t nodeId, const char *payloadType,
  ...
  snprintf(line, sizeof(line),
           "DATA,%s,%u,,,%lu,%s,%u,%u,%u,%d,%d,LEGACY",
           CURRENT_RUN_ID, nodeId, (unsigned long)micros(), payloadType,
           v1, v2, v3, payloadBytes, rssi);
  serialLineAtomic(line);
}
...
void OnDataRecv(const esp_now_recv_info_t *info, const uint8_t *incomingData, int len) {
  ...
  if (type == MSG_ADC) {
    ...
    char legacy[64];
    snprintf(legacy, sizeof(legacy), "%u,%u,%u,%u",
             p.node_id, p.in1, p.in2, p.in4);
    serialLineAtomic(legacy);
  ...
  if (type == MSG_SPEC) {
    ...
    char legacy[120];
    snprintf(legacy, sizeof(legacy), "SPEC,%u,%u,%u,%u,%u,%u,%u",
             s.node_id, s.ch[0], s.ch[1], s.ch[2],
             s.ch[3], s.ch[4], s.ch[5]);
    serialLineAtomic(legacy);
  ...
  if (type == MSG_HUM) {
    ...
    char legacy[40];
    snprintf(legacy, sizeof(legacy), "%u,%u", h.node_id, h.hum_permille);
    serialLineAtomic(legacy);
...
void OnDataSent(const wifi_tx_info_t *info, esp_now_send_status_t status) {
  if (status == ESP_NOW_SEND_SUCCESS && !DEBUG_TX_OK) return;
  char line[96];
  if (info && info->des_addr) {
    snprintf(line, sizeof(line), "# TX %02X:%02X:%02X:%02X:%02X:%02X => %s",
             info->des_addr[0], info->des_addr[1], info->des_addr[2],
             info->des_addr[3], info->des_addr[4], info->des_addr[5],
             status == ESP_NOW_SEND_SUCCESS ? "OK" : "FAIL");
  } else {
    snprintf(line, sizeof(line), "# TX UNKNOWN => %s",
             status == ESP_NOW_SEND_SUCCESS ? "OK" : "FAIL");
  }
  serialLineAtomic(line);
}
...
static void sendCmdToNode(uint8_t node_id, const String &cmdStr) {
  ...
  if (r != ESP_OK) {
    char line[64];
    snprintf(line, sizeof(line), "# ERR esp_now_send=%d", (int)r);
    serialLineAtomic(line);
  } else {
    // No imprimir cada envío correcto durante la corrida: saturaba la UART y
    // podía intercalarse con una fila DATA. Los fallos sí se conservan.
  }
}
