...
% Estado de inferencia de la RNA previamente entrenada.
S.rnaNet = [];
S.rnaLoaded = false;
S.rnaFile = "";
S.previousAction = 1;
...
uibutton(fig,'push','Text','Cargar RNA entrenada', ...
    'Position',[30 550 270 30], ...
    'ButtonPushedFcn',@loadTrainedRNA);
...
algDrop = uidropdown(fig,'Position',[30 330 250 30], ...
    'Items',{'Sensor reactivo','Gradiente local','Quimiosintesis', ...
             ...
             'Metodo hibrido bioinspirado','RNA entrenada'}, ...
...
%% ================= CALLBACKS =================
    ...
    function loadTrainedRNA(~,~)
        [file,path] = uigetfile({'*.mat','Modelo MATLAB (*.mat)'}, ...
            'Seleccione modelo_RNA_navegacion.mat');
        if isequal(file,0), return; end
        try
            loaded = load(fullfile(path,file));
            if ~isfield(loaded,'net')
                error('El archivo seleccionado no contiene la variable net.');
            end
            S.rnaNet = loaded.net;
            S.rnaLoaded = true;
            S.rnaFile = string(fullfile(path,file));
            uialert(fig,['RNA cargada: ' char(file)],'Modelo disponible');
        catch ME
            S.rnaLoaded = false;
            S.rnaNet = [];
            uialert(fig,ME.message,'Error cargando la RNA');
        end
    end

    function exportRouteAndColorMatrix(~,~)
        % Exporta la ruta, el mapa visible y el resumen de la ejecucion.
...
%% ================= SIMULACION =================
    function simulateStep()
        ...
        if S.escapeCounter > 0
            ...
            switch S.algorithm
                ...
                case "RNA entrenada"
                    [desiredTheta,currentPhase] = rnaNavigation(hCurrent);
    ...
    function updateInfo(hCurrent,idxCurrent,currentPhase)
        ...
        info.Value = {
            ...
            ['RNA cargada: ',char(string(S.rnaLoaded))]
            ['Accion RNA: ',num2str(S.rnaAction)]
            ['Confianza RNA: ',num2str(S.rnaConfidence,'%.3f')]
            ['Campo: ',char(S.fieldType)]
            };
    end

%% ================= NAVEGACION CON RNA =================
    function [desiredTheta,phaseName] = rnaNavigation(hCurrent)
        if ~S.rnaLoaded || isempty(S.rnaNet)
            desiredTheta = sensorReactive();
            phaseName = "RNA no cargada - sensor reactivo";
            return;
        end
        ...
        inputVector = [hCurrent;hLag1;hLag2;deltaH;meanH;slopeH; ...
            proxFront;proxLeft;proxRight;S.virtualSignal;S.previousAction];
        probabilities = S.rnaNet(inputVector);
        [S.rnaConfidence,action] = max(probabilities);

        % La regla de seguridad prevalece sobre la inferencia aprendida.
        ...
        switch action
            case 1
                desiredTheta = S.theta;
                phaseName = "RNA - Avanzar";
            case 2
                desiredTheta = S.theta+pi/4;
                phaseName = "RNA - Izquierda";
            case 3
                desiredTheta = S.theta-pi/4;
                phaseName = "RNA - Derecha";
            case 4
                if proxRight>proxLeft
                    desiredTheta = S.theta+pi/2;
                    phaseName = "RNA - Evasion izquierda";
                else
                    desiredTheta = S.theta-pi/2;
                    phaseName = "RNA - Evasion derecha";
                end
            otherwise
                desiredTheta = sensorReactive();
                phaseName = "RNA - Respaldo sensor reactivo";
        end
    end
